...
class MyMPos(ctk.CTk):
    ...
    def mostrar_ventas(self):
        ...
        # Cabecera Tasa
        ctk.CTkLabel(self.contenedor, text=f"Tasa BCV: {self.tasa:.2f} Bs.", text_color="yellow", font=("Arial", 14)).pack(anchor="e", padx=20)

        # Buscador
        self.txt_buscar = ctk.CTkEntry(self.contenedor, placeholder_text="Buscar por código o nombre...", width=500, height=40)
        self.txt_buscar.pack(pady=20)
        self.txt_buscar.bind("<Return>", lambda e: self.agregar_al_carrito())
        ...
        # Visor de Carrito
        self.lista_items = ctk.CTkTextbox(self.contenedor, width=600, height=250, font=("Consolas", 12))
        self.lista_items.pack(pady=10)
        self.lista_items.configure(state="disabled")
    ...
    def agregar_al_carrito(self):
        ...
        if not busqueda:
            return
        prod = self.db.buscar_producto(busqueda)
        if prod:
            # prod = (codigo_barras, nombre, precio_venta, stock)
            codigo, nombre, precio, stock = prod[0], prod[1], float(prod[2]), prod[3]

            if codigo in self.carrito:
                self.carrito[codigo]['cantidad'] += 1
                self.carrito[codigo]['subtotal'] = self.carrito[codigo]['cantidad'] * precio
            else:
                self.carrito[codigo] = {'nombre': nombre, 'precio': precio, 'cantidad': 1, 'subtotal': precio}
    ...
    def _refrescar_vista_carrito(self):
        self.lista_items.configure(state="normal")
        self.lista_items.delete("0.0", "end")
        self.lista_items.insert("0.0", "CANT | PRODUCTO | PRECIO | SUBTOTAL\n" + "-"*60 + "\n")
        total_usd = 0.0
        for codigo, it in self.carrito.items():
            line = f"{it['cantidad']}x | {it['nombre']} | {it['precio']:.2f}$ | {it['subtotal']:.2f}$\n"
            self.lista_items.insert("end", line)
            total_usd += float(it['subtotal'])
        total_bs = total_usd * self.tasa
        self.lista_items.configure(state="disabled")
        self.lbl_total.configure(text=f"TOTAL: {total_usd:.2f}$ / {total_bs:.2f} Bs.")
    ...
    def finalizar_venta(self):
        ...
        if messagebox.askyesno("Confirmar", f"¿Desea cerrar la venta por un total de {self.lbl_total.cget('text')}?"):
            try:
                # Usamos crear_venta para insertar venta y detalles, y restar stock
                metodo = getattr(self, 'metodo_seleccionado', 'efectivo')
                referencia = None
                if metodo in ("transferencia", "pago movil"):
                    referencia = self.entry_referencia.get().strip()
                    if not referencia:
                        messagebox.showwarning("Referencia requerida", "Debes ingresar la referencia para este método de pago.")
                        return

                venta_id = self.db.crear_venta(self.carrito, metodo_pago=metodo, referencia=referencia)
                if venta_id:
                    messagebox.showinfo("Éxito", f"Venta registrada (ID: {venta_id}). Stock actualizado.")
                    # generar recibo PDF
                    try:
                        pdf_path = self.generar_recibo_pdf(venta_id)
                        if pdf_path:
                            if messagebox.askyesno("Imprimir/Abrir", "¿Deseas abrir el recibo en PDF?"):
                                try:
                    ...
                    self.carrito = {}
                    # Verificar productos con stock bajo y avisar
                    try:
                        bajos = self.db.obtener_productos_bajo_stock()
                        if bajos:
                            lines = [f"{r[2]} (Código: {r[1]}): {r[3]} <= Mín {r[4]}" for r in bajos]
                            txt = "Los siguientes productos están en o por debajo del stock mínimo:\n\n" + "\n".join(lines)
                            if messagebox.askyesno("Stock bajo", txt + "\n\n¿Deseas abrir el inventario para revisarlos?"):
                                self.mostrar_inventario()
                    except Exception as e:
                        print(f"Error comprobando stock mínimo: {e}")

                    self.mostrar_ventas()  # Refresca la pantalla de ventas
                else:
                    messagebox.showerror("Error", "No se pudo registrar la venta. Revisa la consola para más detalles.")
            except Exception as e:
